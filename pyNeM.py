...
inputport1 = raw_input("Please specify port/s to scan: ")
if any(c.isalpha() for c in remoteServer) == True: #Domain was entered
    serverList.append(gethostbyname(remoteServer))
else:
    if str(remoteServer).find('-') != -1 or str(remoteServer).find('/') != -1:
        serverList = (parseip(remoteServer))
    else:
        serverList.append(remoteServer)

#------------------------------------------------------------------------------------------------------
for remoteServerIP in serverList:
